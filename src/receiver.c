...
void rx_add_div_iq_samples(RECEIVER *rx, double i0, double q0, double i1, double q1) {
  ...
  // the operator's order is downstream of this point: the analysis, the
  // combine, and both ear splits. The one thing that is not is the raw
  // feed to RX2, which each protocol makes for itself and swaps there.
  //
  if (div_arm_swapped()) {
    double ti = i0, tq = q0;
    i0 = i1;
