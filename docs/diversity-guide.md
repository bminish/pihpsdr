...
|---|---|
| **Window centre / width** | Where to look, measured from the signal you are tuned to — the zero-beat note in CW. Kept separately for Window, Carrier, FSK/Digital and CW, so aiming the carrier tracker does not destroy your wideband window. **Bins you have notched out are excluded** — the analysis runs on the raw antennas, upstream of the notch, so without this a notched heterodyne would still be picked as the signal to work on |
