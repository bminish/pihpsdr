...
|---|---|
| **Window centre / width** | Where to look, measured from the signal you are tuned to — the zero-beat note in CW. Kept separately for Window, Carrier, FSK/Digital and CW, so aiming the carrier tracker does not destroy your wideband window. **Bins you have notched out are excluded**, in every Measure-on setting except RADE V1 — the analysis runs on the raw antennas, upstream of the notch, so without this a notched heterodyne would still be picked as the signal to work on. Notching an interferer is now also a way to *tell the combiner to ignore it*. (RADE V1 is the exception because its correlator is given the raw samples rather than a spectrum, so there are no bins to leave out.) |
...
| **Key detect** (CW / Morse only) | Only measure while something is actually being keyed. The loop compares the strongest bin in the window with the quietest that bin has recently been: Morse stops — between letters, between words, between overs — and a steady carrier does not, so this is what tells them apart. With it off, a heterodyne near your zero beat takes the tracker every time the other station pauses; on the recording this was measured against, the tracker sat on the carrier for **38 % of blocks against 3 %**. Four positions and no slider, because higher is not better: every setting rejects a carrier equally well, and what rises with the setting is the signal strength needed before the loop works at all — a signal *n* dB out of the noise reads about *n* dB here, so on the weakest recording in the set the loop updates on 17 % of blocks at the lowest setting and 4 % at the highest. **Normal (3 dB)** is the lowest setting that gets the whole benefit. It needs a block shorter than a keying element, so it does least at the 6 Hz **Resolution** setting |
