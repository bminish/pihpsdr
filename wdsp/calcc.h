...
//
// LOCAL FIX for an upstream build break, not part of the diversity work.
//
// dl1ycf commit 49892a0b ("Some steps of the big 3.1 update") moved this
// declaration out of here and into calcc.c, where it is file-local, but
// left wdsp/linux_port.c taking its address to name the PS thread. The
// result is that upstream/TEST does not compile:
//
//   linux_port.c:284: error: 'doPSCorrChange' undeclared
//
// Verified against a pristine upstream/TEST worktree, so this is not an
// artefact of merging. Restoring the extern is what the four other thread
// entry points linux_port.c names already rely on. Drop this block once
// upstream fixes it.
//
